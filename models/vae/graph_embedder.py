...
class GraphEmbedder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.ligand_dropout = nn.Dropout(ligand_dropout) if ligand_dropout is not None else None
        self.distance_expansion = nn.ModuleDict(
            {
                ...
                self._to_key("atom", "receptor"): GaussianSmearing(
                    0.0, rec_max_radius//2, distance_embed_dim
                ),
                self._to_key("ligand", "atom"): GaussianSmearing(
                    0.0, rec_max_radius//2, distance_embed_dim
                ),
    ...
    def forward(self, data: HeteroData):
        ...
        for src, _, dst in edges:
            ...
            elif self._to_key(*edge_type) in self.distance_expansion:
        ...
        if self.ligand_dropout is not None:
            data["ligand"].x = self.ligand_dropout(data["ligand"].x)
            data["ligand", "ligand"].edge_attr = self.ligand_dropout(data["ligand", "ligand"].edge_attr)
        return data
